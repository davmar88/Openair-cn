  ...
  const nas_message_decode_status_t  * const decode_status)
{
  ...
  if (create_new_emm_ctxt) {
    ...
    if (!new_emm_ctx) {
      ...
      ue_ctx.emm_cause = EMM_CAUSE_NO_SUITABLE_CELLS;//EMM_CAUSE_ILLEGAL_UE;//Davel
      /*
    ...
    if (rc != RETURNok) {
       ...
       * Do not accept the UE to attach to the network
       */
      // new_emm_ctx->emm_cause = EMM_CAUSE_ILLEGAL_UE; //Davel uncomment
      new_emm_ctx->emm_cause = EMM_CAUSE_NO_SUITABLE_CELLS; //Davel remove line
      rc = _emm_attach_reject (new_emm_ctx); //Davel uncomment
      
    } else {
      /*
       * Performs the sequence: UE identification, authentication, security mode
       */
      //new_emm_ctx->emm_cause = EMM_CAUSE_NO_SUITABLE_CELLS;
      rc = _emm_attach_identify (new_emm_ctx);
    }
  }
  
  //rc = _emm_attach_identify(new_emm_ctx);//Davel
  OAILOG_FUNC_RETURN (LOG_NAS_EMM, rc);
}
  ...
  void *args)
{
  ...
  if (IS_EMM_CTXT_PRESENT_IMSI(emm_ctx)) {
    ...
    if (rc != RETURNok) {
      ...
      emm_ctx->emm_cause = EMM_CAUSE_NO_SUITABLE_CELLS; //EMM_CAUSE_ILLEGAL_UE;//Davel
      // Do not accept the UE to attach to the network
  ...
  rc = _emm_attach_security (emm_ctx);//Davel
  OAILOG_FUNC_RETURN (LOG_NAS_EMM, rc);
}
